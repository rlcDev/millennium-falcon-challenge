...
export class R2d2Command extends CommandRunner {
  ...
  static falconConfig = () => {
    ...
    try {
      ...
      if (args && !args.includes('-h') && args.length >= 1) {
        ...
        value = { autonomy: 0, departure: '', arrival: '', routes_db: '' }; // Faking a config load
      }
    } catch (error) {
  ...
  /**
   * Running the command
   * @param passedParams {Array>} Params
    ...
    if (passedParams.length > 2) {
      ...
      try {
        const empireDto: EmpireDto = JSON.parse(
          readFileSync(this.setRelativePath(passedParams[1]), {
            encoding: 'utf-8',
          }),
        );
  ...
  /**
   * Set relative path smoothly
   * @param currentPath {string} path
   * @private
   */
  private setRelativePath(currentPath: string): string {
