...
export class OddMissionService {
     ...
     *
     * @param empire {Empire}
     * @return The odd x  as 0 < x < 1
     */
    public tellMeTheOdds(empire: Empire): number {
        const completePaths: SpaceTravelPath[] =
            this.getSuccessfulMissionSpaceTravelPaths(this.universeService.galaxy, empire, this.universeService.falcon);
        return completePaths.length === 0
            ? 0
            : this.computeOdd(this.getHuntersCount(completePaths, empire));
    }
    ...
    /**
     * Get all successful travel paths to Endor
     *
     * @param galaxy {Galaxy}
     * @param empire {Empire}
     * @param falcon {Falcon}
